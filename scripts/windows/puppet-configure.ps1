  ...
  Get-Content -Path $ForgeMods | % {
    ...
    if ($modulename -ne '') {
      ...
      do {
        ...
        try {
          & $PuppetPath module install $modveropt $moduleversion --target-dir $ModulesPath $modulename
          $EC = $LASTEXITCODE
...
Get-ChildItem -Path $PackerPuppet -Filter '*.pp' | ? { -not $_.PSIsContainer } | % {
  ...
  do {
    ...
    & $PuppetPath apply ($Manifest.Fullname) "--modulepath=$ModulesPath" --detailed-exitcodes
    $EC = $LASTEXITCODE
    switch ($EC) {
      0 {
